  ...
  async open() {
    ...
    if (!this.cookies) {
      const req = await fetch(
        this.endpoint.replace('ws', 'http').replace('/websocket', ''),
        {
          method: 'HEAD',
        }
      );
    ...
    this.rawSocket = new this.SocketConstructor(
      this.endpoint,
      undefined,
      this.cookies ? { headers: { Cookie: this.cookies } } : undefined
    );
    /*
     *   Calls to `onopen` and `onclose` directly trigger the `open` and
     *   `close` events on the `Socket` instance.
     */
    this.rawSocket.onopen = (e) => {
      this.emit('open');
    };
